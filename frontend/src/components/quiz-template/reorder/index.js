...
export default function App({ quiz, nextQ }) {
  ...
  return (
    ...
    >
      <div
        dir={layoutDir}
        className={`rounded-xl border border-indigo-300 bg-gradient-to-br from-blue-50 to-indigo-100 p-2 shadow-lg pb-[max(0.5rem,env(safe-area-inset-bottom))] sm:p-3 ${
      ...
      >
        ...
        <div className="mt-6 flex flex-col items-center text-center md:mt-8">
          <button
            type="button"
            onClick={() =>
              nextQ({ id: quiz._id, answers: items.map((ele) => ele.id) })
            }
