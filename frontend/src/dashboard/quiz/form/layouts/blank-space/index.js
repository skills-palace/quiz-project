...
function Index({ arrayField }) {
  ...
  const setWord = () => {
    if (popup.type === "word" && words.length > 0) {
      const newItem = {
        id: nanoid(),
        title: words, // Array of words
        type: popup.type,
        mark: mark,
      };
      ...
      setNewWord("");
      setMark(0);
    } else if (popup.type === "text" && newWord.trim() !== "") {
      const newItem = {
        id: nanoid(),
        title: newWord, // Single string for text
        type: popup.type,
        mark: 0,
      };
          ...
          fields.map((item, idx) => (
            <span key={item.id}>
              ...
              <input
                ...
                value={item.type === "word" ? item?.title?.join(", ") : item.title}
              />
              ...
              {item.type === "newline" ? (
                ...
                >
                  {item.type === "word" ? item.title?.join(", ") : item.title}
                  <BiX
        ...
        >
          ...
          {popup.type === "word" && (
            <div>
              ...
              />
              <button
                className="ml-2 p-1 text-white bg-blue-500 rounded"
                onClick={addWord}
                type="button"
              >
                <BiPlus />
              ...
              <div className="mt-2">
                {words.map((word, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gray-200 rounded p-1 mr-1 mb-1"
                  >
                    {word}
                    <BiX
